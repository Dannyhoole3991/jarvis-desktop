...
class HostAgentProcessor(BaseProcessor):
    ...
    def execute_action(self) -> None:
        ...
        # Guard against handing off to the AppAgent with no application window
        # resolved at all — e.g. right after launching an app via a bash
        # command, before its window has actually appeared on the desktop.
        # Without this, AppAgentProcessor.capture_screenshot() crashes with
        # "AttributeError: 'NoneType' object has no attribute
        # 'capture_as_image'" because self.application_window is still None.
        # Retry instead so the next round re-scans the desktop once the
        # window has actually opened.
        if (
            self.status == self._agent_status_manager.ASSIGN.value
            and self.application_window is None
        ):
            utils.print_with_color(
                "The target application window is not open yet (it may still "
                "be launching); retrying instead of handing off to the "
                "AppAgent.",
                "yellow",
            )
